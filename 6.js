...
const LED_WIDTH  = 1000;
const LED_HEIGHT = 1000;

const led = _.times(LED_HEIGHT, i => _.times(LED_WIDTH, i => '0'));

const instructions = _(input.split('\n'))
                    .map(_.trim)
                    .reject(_.isEmpty)
                    .map(i => {
                        const result = i.match(/^(turn on|turn off|toggle)\s(\d+)\,(\d+)\sthrough\s(\d+)\,(\d+)$/);
                        return {
                            action: result[1],
                            x0:     parseInt(result[2]),
                            y0:     parseInt(result[3]),
                            x1:     parseInt(result[4]),
                            y1:     parseInt(result[5])
                        }
                    })
                    .value();

const litLed = _.reduce(instructions, (acc, i) => {
    _.each(_.range(i.y0, i.y1+1), y => {
        _.each(_.range(i.x0, i.x1+1), x => {
            switch (i.action) {
                case 'turn on':
                    acc[y][x] = '1';
                    break;
...
const numLitLights = _.filter(_.flatten(litLed), i => i === '1').length;
console.log(numLitLights);

// BONUS CODE! VISUALIZE THE LED!
...
//     <head>
//         <meta charset="UTF-8">
//         <title>Adventofcode 6</title>
//     </head>
//     <body>
//         <canvas id="canvas" width="${LED_WIDTH}" height="${LED_HEIGHT}" />
//         <script>
...
//             }
//
//             var ledData   = ${JSON.stringify(led)};
//             var canvas    = document.getElementById('canvas');
//             var ctx       = canvas.getContext('2d');
//             var imageData = ctx.getImageData(0, 0, ${LED_WIDTH}, ${LED_HEIGHT});
